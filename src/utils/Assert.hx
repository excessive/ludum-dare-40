...
class AssertionFailure {
	...
	public function new(message:String, parts:Array<AssertionPart>, ?user_msg: String) {
...
// based on the haxe docs
class Assert {
	static public macro function assert(e:Expr, ?msg: String) {
		var s = e.toString();
		var p = e.pos;
		var el = [];
		var descs = [];
		...
		el.push(macro if (!$e) @:pos(p) throw new utils.Assert.AssertionFailure($v{s}, $a{a}, $v{msg}));
		return macro $b{el};
	}
}

...
class Profiler {
	...
	static function render(frame: Frame) {
		GameInput.bind(GameInput.Action.Debug_F3, function() {
			Main.toggle_window(ProfilerUI);
			return true;
		});

		if (!Main.showing_menu(ProfilerUI)) {
			return;
		}
...
class Profiler {
	...
	public static inline function marker(label: String, ?color: SegmentColor) {}
	public static inline function push_block(label: String, ?color: SegmentColor) {}
